...
function Blob(x, y, r, color) {
    ...
    this.update = function () {
        ...
        let mouseitself = createVector(mouseX, mouseY)
        if ( mouseitself.dist( center ) < 100 ) {
            mouse.setMag( (mouseitself.dist( center ) / 100 * 3)  );
        } else {
            mouse.setMag(3);
        }

        
        if ( !(this.pos.x < -GRID && mouse.x < 0) && !(this.pos.x > GRID && mouse.x > 0 ) ) {
            this.pos.x += mouse.x;
        }
        if ( !(this.pos.y < -GRID && mouse.y < 0) && !(this.pos.y > GRID && mouse.y > 0 ) ) {
            this.pos.y += mouse.y;
        }
    };
